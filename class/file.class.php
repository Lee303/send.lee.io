...
define('FILE_STATUS_COMPLETE', 2);
define('FILE_STATUS_ERROR', 3);
...
class file
{
    ...
    public static function create($db)
    {
        $file = new file($db);
        $file->timestamp = time();
        $file->uniq_id = self::new_uniqid();
        $file->status = FILE_STATUS_NEW;
    ...
    public static function get($db, $uniq_id)
    {
        ...
        return (new file($db))->hydrate($results[0]);
    }
    ...
    public static function get_expired($db)
    {
        ...
        if (count($results) > 0)
        {
            ...
            {
                $files[] = (new file($db))->hydrate($result);
            }
        }
    ...
    public function write_stream($handle)
    {
        ...
        if (!$fp = fopen($file_path, "w"))
        {
            ...
            $this->set_status(FILE_STATUS_ERROR, $status_message);
        ...
        $this->set_status(FILE_STATUS_COMPLETE);
    }
